...
class Editor:
    ...
    if is_windows:
        KEYMAP.update({
        "\x1b[H" : KEY_UP,
        "\x1b[\x8d": KEY_SHIFT_UP,
        "\x1b[P" : KEY_DOWN,
        "\x1b[\x91": KEY_SHIFT_DOWN,
        "\x1b[K" : KEY_LEFT,
        "\x1b[M" : KEY_RIGHT,
        "\x1b[G" : KEY_HOME, ## in Linux Terminal
        "\x1b[O" : KEY_END,  ## Linux Terminal
        "\x1b[I": KEY_PGUP,
        "\x1b[Q": KEY_PGDN,
        "\x1b[s": KEY_WORD_LEFT,
        "\x1b[t": KEY_WORD_RIGHT,
        "\x1b[S" : KEY_DELETE,
        "\x1b[\x94" : KEY_BACKTAB, ## Shift Tab
## other keys
        ...
        while True:
            ...
            if in_buffer[0] == '\x1b': ## starting with ESC, must be fct
                while True:
                    in_buffer += self.rd()
                    c = in_buffer[-1]
                    if c == '~' or (ord(c) > 64 and len(in_buffer) > 2):
                        break
