...
## symbols that are shared between instances of Editor
    ...
    def display_window(self): ## Update window and status line
        ...
        self.hilite(1)
        self.wr(Editor.TERMCAP[14 if Editor.width > 40 else 15].format(
    ...
    def line_edit(self, prompt, default, zap=None):  ## better one: added cursor keys and backsp, delete
        push_msg = lambda msg: self.wr(msg + Editor.TERMCAP[13] * len(msg)) ## Write a message and move cursor back
        self.goto(Editor.height, 0)
        self.hilite(1)
        self.wr(prompt)
        self.wr(default)
        ...
        while True:
            ...
            if key == KEY_NONE: ## char to be inserted
                ...
                self.wr(Editor.TERMCAP[13] * pos + ' ' * len(res) + Editor.TERMCAP[13] * len(res))
