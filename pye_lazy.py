...
##   Copy/Delete & Paste, Indent, Un-Indent, Block-Comment
...
## symbols that are shared between instances of Editor
    ...
    def get_input(self):  ## read from interface/keyboard one byte each and match against function keys
        while True:
            in_buffer = self.rd()
            if ord(in_buffer[0]) >= 32 and ord(in_buffer[0]) != 127:
    ...
    def display_window(self, full_screen): ## Update window and status line
...
## update_screen
        ...
        if full_screen:
            i = self.top_line
            lrange = range(Editor.height)
        else:
            i = self.cur_line
            lrange = range(self.row, self.row + 1)
        for c in lrange:
...
## And '$' has to be treated separately, so look for a true EOL match first
    ...
    def edit_loop(self): ## main editing loop
        ...
        while True:
            self.display_window(key != KEY_NONE) 
            if lazy is None:
                self.update_status(0)
                key, char = self.get_input() 
            else:
                lazy.init(period=200, mode=lazy.ONE_SHOT, callback = self.update_status)
                key, char = self.get_input() 
                lazy.deinit()
