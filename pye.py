...
##            pass
    ...
    def set_screen_parms(self):
        ...
        self.height -= 1
        self.scrbuf = [(False,"\x00")] * self.height ## force delete
...
## display Status-Line
    ...
    def handle_edit_keys(self, key): ## keys which change content
        l = self.content[self.cur_line]
        if key == KEY_DELETE:  ## must be first, since 0x7f is in std char range
            if self.mark != None:
...
## prepare content
    ...
    if type(content) == str and content: ## String = non-empty Filename
        ...
        e.content = content ## non-empty list of strings -> edit
## edit
